import { Configuration, CountryCode, DepositoryAccountSubtype, LinkTokenCreateRequest, PlaidApi, PlaidEnvironments, Products } from 'plaid';
...
import { PLAID_ACCESS_TOKEN, PLAID_API_BASE, PLAID_CLIENT_ID, PLAID_PUBLIC_TOKEN, PLAID_SECRET } from './constants';
...
export class PlaidService {
  ...
  async api(
    ...
    try {
      switch(method) {
        case 'post':
          return await firstValueFrom(this.httpService.post(`${this.apiBase}/${url}`, payload).pipe(map(res => res.data)));
        default:
          return await firstValueFrom(this.httpService.get(`${this.apiBase}/${url}`).pipe(map(res => res.data)));
